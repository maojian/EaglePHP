...
var navTab = {
	...
	_closeTab: function(index){
		...
		if ($.fn.xheditor) {
			$("textarea.editor", this._getPanels().eq(index)).xheditor(false);
		}
	...
	_reload: function($tab, flag){
		...
		if (flag && url) {
			...
			if ($tab.hasClass("external")){
				...
				var formData = $panel.find("#pagerForm").serializeArray();
				$panel.loadUrl(url, formData, function(){
	...
	reload: function(url, options){
		...
		if ($panel){
			...
			$.each(formData, function(i, field){
				...
				op.data[field.name] = field.value;
			});
