...
class SplitWord
{
    ...
    {
        $this->addonDicFile = realpath(DATA_DIR.'Dict/words_addons.dic');
        $this->mainDicFile = realpath(DATA_DIR.'Dict/base_dic_full.dic');
        
        if(!file_exists($this->addonDicFile) || !file_exists($this->mainDicFile))
        {
            throw_exception('提取内容关键字的数据词典文件不存在,<br/>请先下载：http://code.google.com/p/eaglephp/downloads/list<br/>如果您不需要此服务，请到后台参数设置处关闭自动提取关键字功能！');
        }
