...
class Sina 
{
	 ...
	 * @return array
	 */
	public function login( $url, $response_type = 'code', $state = NULL, $display = NULL ) {
		$params = array();
	...
	public function getAccessToken( $type = 'code', $keys ) {
		...
		if ( $type === 'token' ) {
			...
			throw_exception('wrong auth type');
		}
		...
		if ( is_array($token) && !isset($token['error']) ) {
			...
			throw_exception('get access token failed.' . $token['error']);
		}
		$token['openid'] = $token['uid'];
		unset($token['remind_in'], $token['uid']);
		return $token;
	}
	...
	{
		$data = $this->show_user_by_id($uid);//根据ID获取用户等基本信息
		return array('name'=>$data['name'], 'gender'=>($data['gender'] == 'm' ? '男' : '女'), 'avatar'=>$data['profile_image_url']);
	}
	...
	public function public_timeline( $page = 1, $count = 50, $base_app = 0 )
	{
		...
		return $this->get('statuses/public_timeline', $params);//可能是接口的bug不能补全
	}
    ...
    public function show_batch( $ids )
	{
		...
		return $this->get('statuses/show_batch', $params);
	}
	...
	public function querymid( $id, $type = 1, $is_batch = 0 )
	{
		...
		return $this->get( 'statuses/querymid',  $params);
	}
	...
	public function queryid( $mid, $type = 1, $is_batch = 0, $inbox = 0, $isBase62 = 0)
	{
		...
		return $this->get('statuses/queryid', $params);
	}
	...
	public function repost_daily( $count = 20, $base_app = 0)
	{
		...
		$params['count'] = intval($count);
		$params['base_app'] = intval($base_app);
		return $this->get('statuses/hot/repost_daily',  $params);
	}
	...
	public function repost_weekly( $count = 20,  $base_app = 0)
	{
		...
		$params['count'] = intval($count);
		$params['base_app'] = intval($base_app);
		return $this->get( 'statuses/hot/repost_weekly',  $params);
	}
	...
	public function comments_daily( $count = 20,  $base_app = 0)
	{
		...
		$params['count'] = intval($count);
		$params['base_app'] = intval($base_app);
		return $this->get( 'statuses/hot/comments_daily',  $params);
	}
	...
	public function comments_weekly( $count = 20, $base_app = 0)
	{
		...
		$params['count'] = intval($count);
		$params['base_app'] = intval($base_app);
		return $this->get( 'statuses/hot/comments_weekly', $params);
	}
	...
	*/
	...
	public function emotions( $type = "face", $language = "cnname" )
	{
		...
		return $this->get( 'emotions', $params );
	}
	...
	public function comments_by_me( $page = 1 , $count = 50, $since_id = 0, $max_id = 0,  $filter_by_source = 0 )
	{
		...
		$params['page'] = $page;
		$params['filter_by_source'] = $filter_by_source;
		return $this->get( 'comments/by_me', $params );
	}
	...
	public function comments_to_me( $page = 1 , $count = 50, $since_id = 0, $max_id = 0, $filter_by_author = 0, $filter_by_source = 0)
	{
		...
		$params['filter_by_author'] = $filter_by_author;
		$params['filter_by_source'] = $filter_by_source;
		return $this->get( 'comments/to_me', $params );
	}
	...
	public function comments_mentions( $page = 1, $count = 50, $since_id = 0, $max_id = 0, $filter_by_author = 0, $filter_by_source = 0)
	{
		...
		$params['filter_by_author'] = $filter_by_author;
		$params['filter_by_source'] = $filter_by_source;
		return $this->get( 'comments/mentions', $params );
	}
	...
	public function domain_show( $domain )
	{
		...
		return $this->get( 'users/domain_show', $params );
	}
	...
	public function users_show_batch_by_id( $uids )
	{
		...
		return $this->get( 'users/show_batch', $params );
	}
	...
	public function friends_by_id( $uid, $cursor = 0, $count = 50 )
	{
		...
		return $this->get( 'friendships/friends', $params );
	}
	...
	public function friends_in_common( $uid, $suid = NULL, $page = 1, $count = 50 )
	{
		...
		$params['count'] = $count;
		$params['page'] = $page;
		return $this->get( 'friendships/friends/in_common', $params  );
	}
	...
	public function bilateral( $uid, $page = 1, $count = 50, $sort = 0 )
	{
		...
		$params['page'] = $page;
		$params['sort'] = $sort;
		return $this->get( 'friendships/friends/bilateral', $params  );
	}
	...
	public function bilateral_ids( $uid, $page = 1, $count = 50, $sort = 0)
	{
		...
		$params['page'] = $page;
		$params['sort'] = $sort;
		return $this->get( 'friendships/friends/bilateral/ids',  $params  );
	}
	...
	public function friends_ids_by_id( $uid, $cursor = 0, $count = 500 )
	{
		...
		$params['cursor'] = $cursor;
		$params['count'] = $count;
		return $this->get( 'friendships/friends/ids', $params );
	}
	...
	public function friends_remark_batch( $uids )
	{
		...
		return $this->get( 'friendships/friends/remark_batch', $params  );
	}
	...
	public function followers_ids_by_id( $uid, $cursor = 0 , $count = 50 )
	{
		...
		$params['count'] = $count;
		$params['cursor'] = $cursor;
		return $this->get( 'friendships/followers/ids', $params  );
	}
	...
	public function followers_active( $uid,  $count = 20)
	{
		...
		$param['uid'] = $uid;
		$param['count'] = $count;
		return $this->get( 'friendships/followers/active', $param);
	}
	...
	public function friends_chain_followers( $uid, $page = 1, $count = 50 )
	{
		...
		$params['count'] = $count;
		$params['page'] = $page;
		return $this->get( 'friendships/friends_chain/followers',  $params );
	}
	...
	public function dm_user_list( $count = 20, $cursor = 0)
	{
		...
		$params['count'] = $count;
		$params['cursor'] = $cursor;
		return $this->get( 'direct_messages/user_list', $params );
	} 
	...
	public function dm_conversation( $uid, $page = 1, $count = 50, $since_id = 0, $max_id = 0)
	{
		...
		$params['count'] = $count;
		$params['page'] = $page;
		return $this->get( 'direct_messages/conversation', $params );
	}
	...
	public function dm_show_batch( $dmids )
	{
		...
		return $this->get( 'direct_messages/show_batch',  $params );
	}
	...
	public function account_profile_basic( $uid = NULL  )
	{
		...
		return $this->get( 'account/profile/basic', $params );
	}
	...
	public function account_education( $uid = NULL )
	{
		...
		return $this->get( 'account/profile/education', $params );
	}
	...
	public function account_education_batch( $uids  )
	{
		...
		return $this->get( 'account/profile/education_batch', $params );
	}
	...
	public function account_career( $uid = NULL )
	{
		...
		return $this->get( 'account/profile/career', $params );
	}
	...
	public function account_career_batch( $uids )
	{
		...
		return $this->get( 'account/profile/career_batch', $params );
	}
	...
	{
		return $this->get('account/get_privacy');
	}
	...
	public function school_list( $query )
	{
		...
		return $this->get( 'account/profile/school_list', $params );
	}
	...
	public function favorites_show( $id )
	{
		...
		$this->id_format($id);
		$params['id'] = $id;
		return $this->get( 'favorites/show', $params );
	}
	...
	public function favorites_by_tags( $tid, $page = 1, $count = 50)
	{
		...
		$params['count'] = $count;
		$params['page'] = $page;
		return $this->get( 'favorites/by_tags', $params );
	}
	...
	public function favorites_tags( $page = 1, $count = 50)
	{
		...
		$params['count'] = $count;
		$params['page'] = $page;
		return $this->get( 'favorites/tags', $params );
	}
	...
	public function trends_is_follow( $trend_name )
	{
		...
		return $this->get( 'trends/is_follow', $params );
	}
	...
	public function hourly_trends( $base_app = 0 )
	{
		...
		return $this->get( 'trends/hourly', $params );
	}
	...
	public function daily_trends( $base_app = 0 )
	{
		...
		return $this->get( 'trends/daily', $params );
	}
	...
	public function weekly_trends( $base_app = 0 )
	{
		...
		return $this->get( 'trends/weekly', $params );
	}
	...
	public function get_tags_batch( $uids )
	{
		...
		return $this->get( 'tags/tags_batch', $params );
	}
	...
	public function verify_nickname( $nickname )
	{
		...
		return $this->get( 'register/verify_nickname', $params );
	}
	...
	public function search_users( $q,  $count = 10 )
	{
		...
		$params['q'] = $q;
		$params['count'] = $count;
		return $this->get( 'search/suggestions/users',  $params );
	}
	...
	public function search_statuses( $q,  $count = 10)
	{
		...
		$params['q'] = $q;
		$params['count'] = $count;
		return $this->get( 'search/suggestions/statuses', $params );
	}
	...
	public function search_schools( $q,  $count = 10,  $type = 1)
	{
		...
		$params['count'] = $count;
		$params['type'] = $type;
		return $this->get( 'search/suggestions/schools', $params );
	}
	...
	public function search_companies( $q, $count = 10)
	{
		...
		$params['q'] = $q;
		$params['count'] = $count;
		return $this->get( 'search/suggestions/companies', $params );
	}
	...
	public function search_at_users( $q, $count = 10, $type=0, $range = 2)
	{
		...
		$params['type'] = $type;
		$params['range'] = $range;
		return $this->get( 'search/suggestions/at_users', $params );
	}
	...
	{
		return $this->get( 'search/statuses', $query );
	}
	...
	{
		return $this->get( 'search/users', $query );
	}
	...
	public function hot_users( $category = "default" )
	{
		...
		return $this->get( 'suggestions/users/hot', $params );
	}
	...
	{   
		...
		$params['page'] = $page;
		$params['count'] = $count;
		return $this->get( 'suggestions/users/may_interested', $params);
	}
	...
	public function suggestions_users_by_status( $content, $num = 10 )
	{
		...
		$params['num'] = $num;
		return $this->get( 'suggestions/users/by_status', $params);
	}
	...
	public function hot_favorites( $page = 1, $count = 20 )
	{
		...
		$params['count'] = $count;
		$params['page'] = $page;
		return $this->get( 'suggestions/favorites/hot', $params);
	}
