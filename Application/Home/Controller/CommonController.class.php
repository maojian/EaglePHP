...
class CommonController extends Controller{
    ...
    public function _initialize() {
    	...
    	$this->assign('case_list', model('case')->field('title,img,url')->where('state=0')->order('rank DESC,id DESC')->limit(4)->select(array('cache'=>true)));
    	...
    	$this->assign('links', model('links')->field('name,url,img')->where('state=0')->order('rank DESC,id DESC')->select(array('cache'=>true)));
