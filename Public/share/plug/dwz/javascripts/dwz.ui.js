...
//	if ($.fn.pngFix) $p.pngFix();
	...
	if ($.fn.swfupload) {
		return $("#spanButtonPlaceholder",$p).each(function(){
			var $this = $(this);
			$this.swfupload();
		});
	}
	...
	if ($.fn.uploadify) {
		$(":file[uploader]", $p).each(function(){
			var $this = $(this);
			var auto = ($this.attr("auto")=="false") ? false : true; //maojian
			var options = {
			...
			if ($this.attr("onSelectOnce")) {
				options.onSelectOnce = DWZ.jsonEval($this.attr("onSelectOnce"));
			}
