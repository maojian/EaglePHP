...
class DbPdo extends Db implements DbInterface{
    ...
    public function __construct($config) {
    	if(!class_exists('PDO')){
    		throw_exception(L('SYSTEM:module.not.loaded', array('PDO')));
    	}
    	$this->config = $config;
    }
    ...
    public function connect($pconnect = false){
    	try{
    		extract($this->config);
   			...
   			if($pconnect){ // 注：此处如果sqlsrv驱动器采用长连接模式会导致内存泄露，出现浏览器崩溃现象，请慎用。
   				$dbparams[PDO::ATTR_PERSISTENT] = true;
   			}
   			$dbparams[PDO::ATTR_ERRMODE] = PDO::ERRMODE_SILENT; // PDO::ERRMODE_EXCEPTION 
